...
class ShallowResnetEncoder(nn.Module):
    """Pytorch module for a resnet encoder
    """

    def __init__(self, num_layers, pretrained, num_input_images=1, **kwargs):
        super(ShallowResnetEncoder, self).__init__()
    ...
    def forward(self, input_image):
        ...
        x = self.encoder.conv1(x)
        x = self.encoder.bn1(x)
        self.features.append(self.encoder.relu(x)) # (64,160,240)
        self.features.append(self.encoder.layer1(self.encoder.maxpool(self.features[-1]))) # (64,80,120)
        self.features.append(self.encoder.layer2(self.features[-1])) # (128,40,60)
        # self.features.append(self.encoder.layer3(self.features[-1]))
        # self.features.append(self.encoder.layer4(self.features[-1]))
